...
export default function TicketPage({ aiProvider, aiModel }: TicketPageProps) {
  ...
  // Latest generated ticket draft = newest assistant message carrying ticket data
  const latestTicketMsg =
  ...
  return (
    <div className="flex h-[calc(100vh-140px)]">
      {/* Sessions History Sidebar Drawer — fixed height, scrolls independently of chat */}
      {showHistorySidebar && (
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full overflow-hidden">
        ...
        <div className="flex items-center gap-2 px-4 py-3 border-b border-slate-100 dark:border-slate-700 shrink-0">
          ...
          <span className="text-sm font-semibold text-slate-800 dark:text-slate-200 truncate">
            {activeSession ? `AI Ticket Chat — ${activeSession.title}` : "AI Ticket Chat"}
          </span>
          ...
          {messages.length === 0 ? (
            <div className="flex flex-col items-center justify-center min-h-[45vh] text-center p-4">
              ...
              <p className="text-xs text-slate-500 mb-4">
                Describe a bug, paste a Google Drive link, or drop a screenshot. AI auto-detects issue type and builds your Jira ticket.
              </p>
              <button
                type="button"
                onClick={() => setInputText("https://drive.google.com/file/d/1dYXrjypQIbuZxheDHJom7UtazP_mUcmi/view bug inpainting when base image from Google Nano Banana Pro fails to remove object")}
              ...
              >
                💡 Example: Nano Banana Inpainting Bug
              </button>
            </div>
          ) : (
                ...
                <div className={`max-w-[85%] space-y-2 ${msg.role === "user" ? "items-end" : "items-start"}`}>
                  <TicketChatBubble
                    msg={
                      msg.role === "assistant" && msg.ticket_result
                        ? { ...msg, ticket_result: undefined, image_base64: undefined }
                        : msg
                    }
                  ...
                  {msg.role === "assistant" && msg.ticket_result && (
                    <p className="flex items-center gap-1 text-[10px] text-slate-400">
                      ...
                      <span>Ticket draft updated below</span>
                    </p>
                  )}
        ...
        {/* Ticket Draft — inline below composer, docks like Gemini canvas */}
        {latestTicketMsg && (
          <div className="shrink-0 max-h-[45vh] overflow-y-auto border-t border-slate-200 dark:border-slate-700 bg-slate-50/40 dark:bg-slate-900/40 px-4 py-4">
