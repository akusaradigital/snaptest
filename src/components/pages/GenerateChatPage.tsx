...
export default function GenerateChatPage({ aiProvider, aiModel }: Props) {
  ...
  return (
    <div className="flex h-[calc(100vh-140px)] gap-4">
      {/* ── LEFT SIDEBAR ─────────────────────────────────────────────────── */}
      {showSidebar && (
        <div className="w-[280px] border-r border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/40 p-4 flex flex-col shrink-0 h-full overflow-hidden">
      ...
      <div className="flex flex-col min-w-0 min-h-0 flex-1 max-w-4xl mx-auto overflow-hidden">
        {/* Toolbar-lite header: session title / AI Copilot + busy spinner */}
        <div className="flex items-center gap-2 px-4 py-2.5 border-b border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/50 shrink-0">
          {!showSidebar && (
            <button
              type="button"
              onClick={() => setShowSidebar(true)}
              className="p-1.5 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-400 transition shrink-0"
          ...
          <span className="text-xs font-bold text-slate-800 dark:text-slate-200 uppercase tracking-wider truncate">
            {activeSession?.title || "AI Copilot"}
          </span>
        ...
        {/* Artifact Workspace — docked inline below composer */}
        {activeResult && (
